												...
												<form method="POST" action="" class="pt-3" id="feesForm" autocomplete="off" enctype="multipart/form-data">
													...
													<div class="modal-body">
														...
														<div class="form-group">
															...
															<input type="text" name="inMatricID" class="form-control" id="inMatricID" value="<?= $_SESSION['user_id']; ?>" required readonly>
														</div>
														<div class="form-group">
													...
													<tbody>
														...
														function showData($conn)
														{
															if (isset($_POST['submitSearch'])) {
																...
																$sql = "SELECT fees.feesID,fees.feesAmount,fees.memberMatricID,member.memberName,fees.feesType,fees.feesEvidence,fees.feesDate FROM fees INNER JOIN member ON fees.memberMatricID = member.memberMatricID WHERE fees.memberMatricID like '%$valueToSearch%' OR fees.feesID like '%$valueToSearch%' OR fees.feesDate like '%$valueToSearch%' OR member.memberName like  '%$valueToSearch%' OR fees.feesType like '%$valueToSearch%'";
																$query = mysqli_query($conn, $sql);
															} else {
																$sql = "SELECT fees.feesID,fees.feesAmount,fees.memberMatricID,member.memberName,fees.feesType,fees.feesEvidence,fees.feesDate FROM fees INNER JOIN member ON fees.memberMatricID = member.memberMatricID";
																$query = mysqli_query($conn, $sql);
															}
															$i = 0;
															foreach ($query as $row) {
																
																if ($row['memberMatricID'] == $_SESSION['user_id']) {
																	$i++;
																	echo
																	"<tr>
																	<td>$i</td>
